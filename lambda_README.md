...
# Production build only with prefixed-paths
...
Create a folder called `test` under `rem/lambdal/client/templates`. Copy these files from the `gatsby/www/public`  folder into it:

* test/index.html: the main entry
* test/404.html: the 404 page
* test/benchmarks/*: the benchmarks pages
* test/gpus/*: the gpus pages
* test/models/*: the models pages
* test/statics/* the resources, including images and css
...
- [ ] Images and CSS are not configured correctly for Lambda
- [ ] Hacky workaround for sidebar in `gatsby/www/src/components/page-with-sidebar.js`. Meanwhile the `benchmarks` page and the `gpus` page still do not display sidebar correctly
- [ ] Hacky `re_path` in `lambdal/svr/urls.py`
